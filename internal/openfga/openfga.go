...
import (
	...
	"github.com/jon-whit/openfga-authorizer/internal/util"
...
func (o *TupleStoreAndChecker) CheckOne(ctx context.Context, tuple Tuple, contextualTuples []Tuple) (bool, error) {
	...
	resp, err := o.fgaClient.Check(ctx, &openfgav1.CheckRequest{
		...
		TupleKey:             tupleToOpenFGACheckTuple(tuple),
...
func (o *TupleStoreAndChecker) WriteTuples(ctx context.Context, writes, deletes []Tuple) error {
	...
	for i := 0; i < totalLen; {
		...
		var deleteKeys []*openfgav1.TupleKeyWithoutCondition
		for _, delete := range util.Map(deletes[deletesStart:deletesEnd], tupleToOpenFGA) {
			deleteKeys = append(deleteKeys, tuple.TupleKeyToTupleKeyWithoutCondition(delete))
		}

		if deletesStart != deletesEnd {
			req.Deletes = &openfgav1.WriteRequestDeletes{
				TupleKeys: deleteKeys,
			}
		}
...
func tupleToOpenFGACheckTuple(tuple Tuple) *openfgav1.CheckRequestTupleKey {
...
// can be used.
func tupleFilterToOpenFGA(filter *zanzibar.TupleFilter) *openfgav1.ReadRequestTupleKey {
